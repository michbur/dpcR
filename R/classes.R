...
#'
setClass("dpcr", contains = "matrix", representation(
  .Data = "matrix",
  n = "integer",
  exper = "factor",
  replicate = "factor",
...
construct_dpcr <- function(data, n, exper = "Experiment1",
  ...
  if (length(n) != ncol(data)) {
    if (length(n) == 1) {
      message(paste0(
        "The assumed number of partitions in each run is equal to ",
        n, "."
      ))
      n <- rep(n, ncol(data))
    } else {
  ...
  if (length(v) != ncol(data)) {
    if (length(v) == 1) {
      message(paste0(
        "The assumed volume of partitions in each run is equal to ",
        v, "."
      ))
      v <- rep(v, ncol(data))
    } else {
  ...
  if (length(uv) != ncol(data)) {
    if (length(uv) == 1) {
      message(paste0(
        "The assumed volume uncertainty in each run is equal to ",
        uv, "."
      ))
      uv <- rep(uv, ncol(data))
    } else {
...
create_adpcr <- function(data, n, exper = "Experiment1",
  ...
  result <- construct_dpcr(
    data = data, n = n, exper = exper,
