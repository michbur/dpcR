...
setMethod("moments", signature(input = "numeric"), function(input) {
  ...
  calc_moms(data.frame(
    experiment = "Unknown", replicate = "Unknown",
    k = input[1], n = input[2]
  ))
})
...
setMethod("moments", signature(input = "matrix"), function(input) {
  ...
  calc_moms(data.frame(
    experiment = 1L:nrow(input), replicate = "Unknown",
    k = input[, 1], n = input[, 2]
  ))
})
