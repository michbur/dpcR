...
#' @export
#' @keywords classes
setClass("count_test", representation(
...
setMethod(
  ...
  function(object) {
    ...
    signif_stars <- symnum(slot(object, "test_res")[, "p_value"],
      corr = FALSE, na = FALSE,
    ...
    cat("---\nSignif. codes:  ", attr(signif_stars, "legend"),
      sep = "",
...
setMethod("plot", signature(x = "count_test"), function(x, aggregate = FALSE,
  ...
  if (aggregate) {
    ...
    sapply(1L:nlevels(group_coef[["group"]]), function(i) {
      ...
      points(lab_pos, group_coef[lab_pos, "lambda"],
        col = colors[i],
      ...
      axis(
        side = 3, labels = rep(levels(group_coef[["group"]])[i], length(lab_pos)),
