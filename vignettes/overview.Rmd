...
**dpcR** is an R package designed to perform an analysis of digital PCR experiments. This vignette covers the most important features of the package and should be used as an addendum to the manual.

Our package employs the nomenclature MIQUE guidelines for digital PCR. $\lambda$ is the mean number of molecules per partition. Total number of partitions is given by $n$. $k$ is the number of positive partitions.

$$
\lambda = - \ln \left(1 - \frac{k}{n} \right)
$$

We firstly describe `dpcr` class, a parent class for all classes designed to contain digital PCR data. Further, we inspect closer `adpcr` class responsible for array-based digital PCR experiments (all experiments, where output data has precise localisation in two dimensions).

TO DO ddpcr

Although qPCR experiments usually does not follow the fundamental assumption of digital PCR (the mean number of template molecule per partition smaller than 1), we present a possibility of analysis results from high-throughput qPCR systems using the methodology typical for dPCR. `qpcr2pp` function converts qPCR data to `qdpcr` object, which also has its own section.
...
Although, this class is designed to contain results from all possible digital PCR experiments, user will interacts mostly with its inheritors as `adpcr` or `ddpcr`.

`dpcr` is S4 object. If you are not familiar with this object system, learn below how to extract elements from the slots of S4 object:

```{r eval=TRUE}
# below we have S4 objecty
...
Digital PCR data is always a matrix (see `?matrix` in R). Columns and rows represent respectively individual runs and their data points. Since data points are not always equivalent to partitons, the true number of partitions for each run is always defined in slot *n*.  
...
sample_adpcr <- bind_dpcr(sim_adpcr(m = 100, n = 765, times = 100, pos_sums = FALSE, n_panels = 1), 
                          rename(sim_adpcr(m = 100, n = 763, times = 100, pos_sums = FALSE, 
                                           n_panels = 1), 
                                 exper = "Experiment2"))
```
...
In case of **fluo** and **tnp** types, the number of data points in *.Data* slot is almost never equal to the real number of paritions `dpcr`.
...
`Summary` method produces a tabelarized summary of any `dpcr` object. It calculates &#955; and its confidence intervals using both Dube's and Bhat's method. The estimated number of template molecules (m) is also computed using the following relationship:
...
```{r eval=TRUE}
summary(sample_adpcr)

# save summary data without printing it
summ <- summary(sample_adpcr, print = FALSE)
...
Since the `dpcr` objects tends to have many data points and quickly clutter the console, the `show` function prints only first 5 rows of *.Data*, number of ommited rows and the type of the object.
...
If the output if digital PCR system has exact locations in two-dimensional space, it belongs to the `adpcr` class. It is the case for all digital PCR experiments conducted on panels, arrays and so on. The `adpcr` object inherits from `dpcr` objects, but has special slots specyfying the dimensions and their names.

The planar representation of `adpcr` objects is create by `adpcr2panel` function.

```{r eval=TRUE}
adpcr2panel(adpcr_experiments)
```

## Array visualisation

Data from digital PCR arrays can be easily visualized using the `plot_panel` function.
...
```{r eval=TRUE}
# test panel function perform a test for each experiment in apdr object.
test_panel(adpcr_experiments)
```

Further tests are avaible in the **spatstat** package, which is also utilizing S4 object system. The exchange of data between **dpcR** and **spatstat** packages is streamlined by the `adpcr2ppp` function, which converts `adpcr` object to [ppp](http://finzi.psych.upenn.edu/library/spatstat/html/ppp.object.html) class taking into account spatial coordinates of positive partitions.

# `qdpcr` class

The unique feature of **dpcR** package is conversion of qPCR data to dPCR-like experiments. The qPCR data should be in format used by the **qpcR** package (see [qpcR_datasets](http://finzi.psych.upenn.edu/library/qpcR/html/qpcR_datasets.html])), where columns represents particular experiments and one column contains cycle number.
...
`qdpcr` inherits from `dpcr` objects and may be analyzed using abovementioned methods. Moreover, the converted data may visualised using the `plot` method.
