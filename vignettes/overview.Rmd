...
```{r eval=TRUE}
#comparim seedson of the experiments using GLM
#set a ran
set.seed(54321)
...
The dPCR experimentes are compared pairwise using the uniformly most powerful (UMP) ratio test [@fay_2010]. Furthermore, computed p-values are adjusted using Benjamini & Hochberg correction [@benjamini_1995] to control family-wise error rate.
...
The generally advices Wilson's confidence intervals [@brown_2001] are computed independently for every dPCR experiment. To ensure that all confidence intervals simultaneously contain true value of lambda 

and adjusted using Dunn -- &#352;id&#225;k correction, where:

$$
\alpha_{\text{adj}} = 1 - (1 - \alpha)^\frac{1}{T}
$$

Such intervals are wider, but ensure that confidence intervals simultaneously contain true value of lambda.
