...
`extract_run` is an equivalent of [Extract](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Extract.html). It extracts one or more runs from the `dpcr` objects preserving other properties (as an adequate replicate ID and so on).
...
Import functions accessible under `read_dpcr` covers many common dPCR systems. To further extend the scope of our software, we propose a universal dPCR data exchange format REDF (raw exchange digital PCR format). 

## REDF

REDF (Raw Exchange Digital PCR Format) has tabular structure. Each run of dPCR 
is described using following properties:
...
8. **panel_id**: id of the panel (integer).
