...
* *.Data* - matrix containing data from digital PCR runs. Its type is further specified by the *type* slot (see below).  
...
#it is alsop possible to bind a list of dpcr objects... 
three_runs_list <- bind_dpcr(list(single_run, single_run, single_run))
#... which may be useful in loops
...
`extract_dpcr` is an equivalent of [Extract](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Extract.html). It extracts one or more runs from the `dpcr` objects preserving the other properties.
...
we see the expected number of partitions.

It is especially important in case of fluorescence digital PCR data, where one droplet may have assigned few data points.
